...
def perform_classification(df_aligned):
    # Remove the specified columns
    X = df_aligned.drop(columns=['hypo_label', 'glucose', 'hr', 'flag', 'Time'])
    X.columns = X.columns.astype(str)  # Convert all feature names to strings
    y = df_aligned['hypo_label']

    # Lists to store metrics for raw and z-score normalized data
    metrics_raw = {'precision': [], 'recall': [], 'f1_score': []}
    metrics_z_score = {'precision': [], 'recall': [], 'f1_score': []}

    imputer = SimpleImputer(strategy='mean')

    # Perform the classification with 5 repetitions
    for i in range(5):
        # Split the data
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3)

        # Initialize the Bagging classifier with 25 trees
        bagging_clf = BaggingClassifier(base_estimator=DecisionTreeClassifier(), n_estimators=25)
        ...
        y_pred = bagging_clf.predict(X_test_imputed)

        # Evaluate the classifier
        precision, recall, fscore, _ = precision_recall_fscore_support(y_test, y_pred, average='binary')

        # Store the metrics
        metrics_raw['precision'].append(precision)
        metrics_raw['recall'].append(recall)
        metrics_raw['f1_score'].append(fscore)

        # Repeat the process for z-score normalized data
        X_train_z = zscore(X_train)
        X_test_z = zscore(X_test)
        bagging_clf.fit(X_train_z, y_train)
        y_pred_z = bagging_clf.predict(X_test_z)
        precision_z, recall_z, fscore_z, _ = precision_recall_fscore_support(y_test, y_pred_z, average='binary')
        metrics_z_score['precision'].append(precision_z)
        metrics_z_score['recall'].append(recall_z)
        metrics_z_score['f1_score'].append(fscore_z)

    # Calculate the mean performance metrics
    for metric in metrics_raw.keys():
        metrics_raw[metric] = np.mean(metrics_raw[metric])
        metrics_z_score[metric] = np.mean(metrics_z_score[metric])

    # Print the results
    print("Raw Data Metrics:")
    print(metrics_raw)
    print("\nZ-Score Normalized Data Metrics:")
    print(metrics_z_score)
...
if __name__ == "__main__":
    ...
    perform_classification(df_aligned)